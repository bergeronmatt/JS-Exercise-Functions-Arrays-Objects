...
function getCarInfoByIndex(inventory, index) {
  const currentCar = inventory[index];
  return 'This is a' + currentCar.car_make + currentCar.car_model;
}
...
function getLastCarInfo(inventory) {
  const currentCar = inventory[inventory.length - 1];
  return `The car is a ${currentCar.car_make} ${currentCar.car_model}`;
}
...
function getGermanCars(inventory) {
  ...
  for (let i = 0; i < inventory.length; i++) {
    ...
    if (
      currentCar.car_make === "Audi" ||
      currentCar.car_make === "Mercedes-Benz" ||
      currentCar.car_make === "Volkswagen" ||
      currentCar.car_make === "BMW"
    ) {
